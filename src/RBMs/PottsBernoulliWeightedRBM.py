...
class RBM:
    ...
    def __init__(self, 
        ...
        self.model_stamp = f'PottsBernoulliWeightedRBM-{timestamp}'                 # marker that identifies the kind of model
        self.file_stamp = ''                                                        # path and name of the model's file
    ...
    def computeDeltaWeighted(self, V : torch.Tensor, w : torch.Tensor) -> torch.Tensor:
        """Used for computing the gradient of the Likelihood wrt the visible fields weighted on the data.
        ...
        Returns:
            torch.Tensor: Weighted frequencies of the colors at each site.
        """
        
        Ns = V.shape[0]
        delta = ((V == self.all_states) * w.reshape(1, Ns, 1)).sum(1).type(self.dtype) / w.sum()
        delta = torch.clamp(delta, min=self.eps, max=1-self.eps) # (num_states, Nv)
    ...
    def computeDeltaHWeighted(self, V : torch.Tensor, H : torch.Tensor, w : torch.Tensor) -> torch.Tensor:
        """Used for computing the gradient of the Likelihood wrt the weights
        of the interaction visibles-hiddens, weighted on the data.

        Args:
            V (torch.Tensor): Visible variables.
            H (torch.Tensor): Hidden variabels.
            w (torch.Tensor): Weights.

        Returns:
            torch.Tensor: Weighted average tensor product between visible and hidden variables.
        """

        Ns = V.shape[0]
        delta = ((V == self.all_states) * w.reshape(1, Ns, 1)).type(self.dtype) # (num_states, Ns, Nv)
        delta = torch.clamp(delta, min=self.eps, max=1-self.eps)

        return torch.tensordot(delta, H, dims=([1],[0])) / w.sum() # (num_states, Nv, Nh)
    ...
    def computeHWeighted(self, H : torch.Tensor, w : torch.Tensor) -> torch.Tensor:
        """Used for computing the gradient of the Likelihood wrt the hidden fields, weighted on the data.
        ...
        Returns:
            torch.Tensor: Weighted average of the hidden variables.
        """
        
        Ns = H.shape[0]
        return torch.sum(H * w.reshape(Ns, 1), 0) / w.sum() # (1, Nh)
    ...
    def updateWeights(self, v_pos : torch.Tensor, h_pos : torch.Tensor,
        ...
        self.W += self.lr * (self.computeDeltaHWeighted(v_pos, h_pos, w) - self.computeDeltaH(v_neg, h_neg))
        self.W -= self.W.mean(0) # Gauge fixing
        self.vbias += self.lr * (self.computeDeltaWeightd(v_pos, w) - self.computeDelta(v_neg))
        self.hbias += self.lr * (self.computeHWeighted(h_pos, w) - self.computeH(h_neg))
    ...
    def updateWeightsCentered(self, v_pos : torch.Tensor, h_pos : torch.Tensor,
        ...
        self.deltaDataAv = self.computeDeltaWeighted(v_pos, w)
        self.hidDataAv = self.computeHWeighted(h_pos, w)
        deltaGenAv = self.computeDelta(v_neg)
        hidGenAv = self.computeH(h_neg)

        # centered variables
        delta_c_pos_weighted = ((v_pos == self.all_states).type(self.dtype).transpose(0, 1) - self.deltaDataAv) * w.reshape(Ns, 1, 1) # (Ns, num_states, Nv)
        ...
        # gradients
        dW = torch.tensordot(delta_c_pos_weighted, hid_c_pos, dims=[[0], [0]]) / w.sum() - torch.tensordot(delta_c_neg, hid_c_neg, dims=[[0], [0]]) / delta_c_neg.shape[0]
