...
class RBMdataset(Dataset):
    def __init__(self, file_path, dataset='train'):
        ...
        self.data = torch.tensor(f[dataset][()]).type(torch.float32)
    ...
    def get_dataset_mean(self):
        return torch.sum(self.data.type(torch.float32) * self.weights.unsqueeze(-1)) / self.weights.sum()
...
def initVisBias(dataset : torch.utils.data.Dataset) -> torch.Tensor:
    ...
    freq = (w @ X).type(torch.float32) / w.sum()
    freq = torch.clamp(freq, min=eps, max=1. - eps)
    
    return torch.log(freq) - torch.log(1. - freq)
                          
# import command-line input arguments
parser = argparse.ArgumentParser(description='Train an RBM with binary variables both in the visible and hidden layer. The training is weighted.')
...
parser.add_argument('--n_mb',               type=int,   default=512,        help='(Defaults to 512). Minibatch size.')
parser.add_argument('--n_pcd',              type=int,   default=512,        help='(Defaults to 512). Number of permanant chains.')
...
RBM = importlib.import_module('BernoulliBernoulliWeightedRBM').RBM
...
if args.train_mode == 'new':
    ...
    rbm = RBM(num_visible=Nv, num_hidden=args.Nh, device=device)
